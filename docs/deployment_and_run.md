...
- Knowledge and AI research routes are available under `/api/knowledge/*` and `/api/ai/research|market-brief`.
- Knowledge retrieval is lexical-first today, with vector retrieval explicitly feature-flagged (`MARKET_AI_KNOWLEDGE_VECTOR_ENABLED`).
...
```bash
SKIP_BACKUP=1 ./scripts/deploy_linux.sh .env.production
SKIP_BOOTSTRAP=1 ./scripts/deploy_linux.sh .env.production
BACKEND_URL=http://127.0.0.1:18000 FRONTEND_URL=http://127.0.0.1:14173 ./scripts/deploy_linux.sh .env.production
```
...
## Knowledge + AI Research Endpoints
- `POST /api/knowledge/ingest`
- `GET /api/knowledge/recent`
- `GET /api/knowledge/search`
- `GET /api/knowledge/documents/{document_id}`
- `POST /api/ai/research`
- `POST /api/ai/contextual-analyze`
- `POST /api/ai/market-brief`

These are authenticated in production when `MARKET_AI_AUTH_ENABLED=1`. Example:
```bash
TOKEN="$(curl -s http://127.0.0.1:8000/auth/login -H 'content-type: application/json' -d '{"username":"admin","password":"<password>"}' | python -c "import sys, json; print(json.load(sys.stdin)['access_token'])")"
curl -s http://127.0.0.1:8000/api/knowledge/recent -H "Authorization: Bearer $TOKEN"
```

New env toggles:
- `MARKET_AI_KNOWLEDGE_DEFAULT_LIMIT`
- `MARKET_AI_KNOWLEDGE_MAX_LIMIT`
- `MARKET_AI_KNOWLEDGE_VECTOR_ENABLED`
- `MARKET_AI_KNOWLEDGE_VECTOR_MIN_SCORE`
- `MARKET_AI_KNOWLEDGE_READ_RATE_LIMIT_PER_MIN`
- `MARKET_AI_KNOWLEDGE_WRITE_RATE_LIMIT_PER_MIN`
- `MARKET_AI_AI_RESEARCH_RATE_LIMIT_PER_MIN`
- `MARKET_AI_RESEARCH_NOTIFY_TELEGRAM`
...
Rollback safety:
1. Keep the pre-deploy runtime backup (`scripts/backup_runtime.py`) generated by `scripts/deploy_linux.sh` unless `SKIP_BACKUP=1` was set.
2. If a deploy fails after migration, restore from backup plus PostgreSQL backup (`pg_dump` / managed snapshot).
3. Redeploy the previous git revision:
```bash
git checkout <previous-commit>
docker compose --env-file .env.production up -d --build
```
4. Confirm `/health` and `/ready` before reopening traffic.
