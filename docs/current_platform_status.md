...
- Backend now includes broker-managed trading surfaces, alert history, signal history, and execution persistence around the external broker path.
...
- Broker status is additive, broker-managed, and disabled by default unless explicitly configured.
...
- Continue hardening broker-managed reconciliation flows across broker paper/live environments before any execution expansion.
