...
_best_setup_cache: dict | None = None
_best_setup_cache_time: float = 0.0
_BEST_SETUP_TTL_SECONDS: float = 300.0  # reload every 5 minutes
...
def _load_best_setup_map():
    global _best_setup_cache, _best_setup_cache_time
    now = _time.monotonic()
    if _best_setup_cache is not None and (now - _best_setup_cache_time) < _BEST_SETUP_TTL_SECONDS:
        return _best_setup_cache
    ...
    _best_setup_cache = best_map
    _best_setup_cache_time = _time.monotonic()
    return best_map


def invalidate_best_setup_cache():
    """Force reload of optimizer best-setup CSV on next call."""
    global _best_setup_cache, _best_setup_cache_time
    _best_setup_cache = None
    _best_setup_cache_time = 0.0
