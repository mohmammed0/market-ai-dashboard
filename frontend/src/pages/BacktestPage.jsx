...
export default function BacktestPage() {
  ...
  const {
    ...
    error,
    submit,
  } = useJobRunner(jobType, { recentLimit: 6 });
  ...
  async function onSubmit(values) {
    ...
    await submit(() => (
      mode === "vectorbt"
        ? runVectorbtBacktest(payload)
        : mode === "classic"
          ? runBacktest(payload)
          : runModelBacktest({ ...payload, mode })
    ));
  }
  ...
  return (
    <PageFrame
      ...
      description="تشغيلات backtest تقبل الطلب فوراً، تُتابَع كمهام خلفية عند المسارات الثقيلة، ثم تعود لنفس سطح النتائج بعد الاكتمال."
      eyebrow="بحث الاستراتيجية"
      headerActions={<StatusBadge label={mode.toUpperCase()} tone="accent" />}
    >
      <FilterBar
        title="طلب الاختبار التاريخي"
        description="الوضع الكلاسيكي وVectorBT يعملان الآن ضمن دورة قبول وتتبع مهام واضحة، بينما تبقى المسارات الذكية على نفس العرض الحالي."
        action={<StatusBadge label={isWorking ? "جارٍ التشغيل" : "الاستراتيجية جاهزة"} tone={isWorking ? "warning" : "subtle"} />}
      >
        <form className="analyze-form filter-form" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="form-actions">
            <button className="primary-button" type="submit" disabled={isWorking}>
              {isWorking ? "جارٍ التشغيل..." : `تشغيل اختبار ${mode === "vectorbt" ? "VectorBT" : mode === "classic" ? "كلاسيكي" : mode.toUpperCase()}`}
            </button>
          </div>
      ...
      <JobRunPanel
        className="span-5"
        title="تتبّع المهمة"
        description="قبول الطلب، التقدم، الملخص، وآخر التشغيلات لهذا المسار."
      ...
      <SectionCard
        ...
        description="ملخص واضح لنتائج backtest الحالي أو آخر مهمة اكتملت."
      >
        ...
        {result ? (
          <>
            <SummaryStrip
              items={[
                { label: "Engine", value: result.engine ?? (mode === "classic" ? "classic" : mode) },
                { label: "Instrument", value: result.instrument ?? "-" },
                { label: "Trades", value: result.trades ?? result.classic_summary?.trades ?? "-" },
                { label: "Win Rate %", value: result.win_rate_pct ?? result.overall_win_rate_pct ?? result.classic_summary?.overall_win_rate_pct ?? "-" },
              ]}
            />
