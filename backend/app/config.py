...
OLLAMA_MODEL = os.getenv("OLLAMA_MODEL", "mistral:7b-instruct-v0.3-q4_K_M").strip() or "mistral:7b-instruct-v0.3-q4_K_M"
...
AUTONOMOUS_ANALYSIS_SYMBOL_LIMIT = int(os.getenv("MARKET_AI_AUTONOMOUS_ANALYSIS_SYMBOL_LIMIT", "12"))
AUTONOMOUS_TRAIN_SYMBOL_LIMIT = int(os.getenv("MARKET_AI_AUTONOMOUS_TRAIN_SYMBOL_LIMIT", "24"))
AUTONOMOUS_HISTORY_LOOKBACK_DAYS = int(os.getenv("MARKET_AI_AUTONOMOUS_HISTORY_LOOKBACK_DAYS", "45"))
AUTONOMOUS_INCLUDE_DL = os.getenv("MARKET_AI_AUTONOMOUS_INCLUDE_DL", "1").strip().lower() not in {"0", "false", "no"}
AUTONOMOUS_REFRESH_UNIVERSE = os.getenv("MARKET_AI_AUTONOMOUS_REFRESH_UNIVERSE", "1").strip().lower() not in {"0", "false", "no"}
ENABLE_CONTINUOUS_LEARNING = os.getenv("MARKET_AI_ENABLE_CONTINUOUS_LEARNING", "1").strip().lower() not in {"0", "false", "no"}
...
AUTOMATION_ALERT_SYMBOL_LIMIT = int(os.getenv("MARKET_AI_ALERT_SYMBOL_LIMIT", "30"))
AUTOMATION_BREADTH_SYMBOL_LIMIT = int(os.getenv("MARKET_AI_BREADTH_SYMBOL_LIMIT", "40"))
AUTOMATION_WATCHLIST_SYMBOL_LIMIT = int(os.getenv("MARKET_AI_WATCHLIST_SYMBOL_LIMIT", "24"))
...
DEFAULT_SAMPLE_SYMBOLS = [
    symbol.strip().upper()
    for symbol in os.getenv("MARKET_AI_SAMPLE_SYMBOLS", "AAPL,MSFT,NVDA,SPY").split(",")
    if symbol.strip()
]
...
AUTO_TRADING_UNIVERSE_PRESET = os.getenv("MARKET_AI_AUTO_TRADING_UNIVERSE_PRESET", "TOP_500_MARKET_CAP").strip().upper()
